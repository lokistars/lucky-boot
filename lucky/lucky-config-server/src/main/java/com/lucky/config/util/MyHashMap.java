...
public class MyHashMap<K, V> implements Map<K, V>, Cloneable, Serializable {
     ...
     * 基本哈希 bin 节点，用于大多数条目。 （参见下面的 TreeNode 子类，以及 MyLinkedHashMap 中的 Entry 子类。）
     */
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((p = tab[i = (n - 1) & hash]) == null) {
            ...
            if (p.hash == hash &&
                ...
                // 发送hash碰撞后进入else,找到最后一个节点,或最后一个节点和存储key相同退出循环
                for (int binCount = 0; ; ++binCount) {
                    // 找到最后一个节点
                    if ((e = p.next) == null) {
                        // 将要存储的数据设置到上一个节点的next上,尾插法
                        p.next = newNode(hash, key, value, null);
                        // -1 for 1st,链表长度大于等于7转换红黑树
            ...
            // 当前节点不等于null,存储key和连表key相同可能出现, 替换当前节点的value值
            if (e != null) {
                V oldValue = e.value;
        ...
        // 记录map新增次数, 判断数组长度是否超过阈值是否要进行扩容,threshold为二的次方
        ++modCount;
        if (++size > threshold) {
            resize();
        }
    ...
    final Node<K, V>[] resize() {
        // oldTab 当前数组,oldCap 当前数组长度 oldThr 容量负载因子
        Node<K, V>[] oldTab = table;
        int oldCap = (oldTab == null) ? 0 : oldTab.length;
        int oldThr = threshold;
        int newCap, newThr = 0;
        // 当前数组长度大于0, 大于最大容量直接返回,否则扩充一倍空间
        if (oldCap > 0) {
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        if (mappings < 0) {
            ...
            // what we're actually creating. 1.8
